    ...
    void FixedUpdate()
    {
            ...
            if (targetPlayer.transform.position.x < limiteIzq || targetPlayer.transform.position.x > limiteDer)
            {
                if (targetPlayer.transform.position.x < limiteIzq)
                {
                    ...
                    transform.position = new Vector3(limiteIzq + offsetX, targetPlayer.transform.position.y, -10f);
                }
                if (targetPlayer.transform.position.x > limiteDer)
                {
                    transform.SetParent(null);
                    transform.position = new Vector3(limiteDer + offsetX, targetPlayer.transform.position.y, -10f);
                }
            }
            else
            {
                transform.SetParent(targetPlayer.transform);
                transform.localPosition = new Vector3(offsetX, 0f, -10f);
            }
