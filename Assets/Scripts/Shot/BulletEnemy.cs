...
public class BulletEnemy : MonoBehaviour
{
    ...
    public int speed;
    private EnemyController enemigoOrigen;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.gameObject.CompareTag("Player") && collision != null && !collision.gameObject.GetComponent<Player>().gamerover)
        {
            ...
            foreach(GameObject bala in GameObject.FindGameObjectsWithTag("bulletEnemy"))
            {
                Destroy(bala);
            }
            float direccionX = 1f;
    ...
    public void EstablecerEnemigoOrigen(EnemyController enemigo)
    {
